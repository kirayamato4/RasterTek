...
bool FontShader::Init( ID3D11Device * pDevice, HWND hWnd )
{
	return InitShader(
		pDevice,
		hWnd,
		L"Shader/TextureVertexShader.hlsl",
		L"Shader/FontPixelShader.hlsl"
	);
}
...
bool FontShader::Render( ID3D11DeviceContext * pDeviceContext, int indexCount, const MatrixBuffer & matrixBuffer, ID3D11ShaderResourceView * pTexture, XMFLOAT4 fontColor )
{
	if( !SetShaderParam( pDeviceContext, matrixBuffer, pTexture, fontColor ) )
		return false;
...
bool FontShader::InitShader( ID3D11Device * pDevice, HWND hWnd, WCHAR * vsFileName, WCHAR * psFileName )
{
	HRESULT hr = E_FAIL;
...
#pragma region FONT_COLOR_BUFFER
	D3D11_BUFFER_DESC fontBufferDesc;
	ZeroMemory( &fontBufferDesc, sizeof( fontBufferDesc ) );
	fontBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	fontBufferDesc.ByteWidth = sizeof( FontColorBuffer );
	fontBufferDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	fontBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	fontBufferDesc.MiscFlags = 0;
	fontBufferDesc.StructureByteStride = 0;

	hr = pDevice->CreateBuffer( &fontBufferDesc, nullptr, &m_pFontColorBuffer );
	HR_ERROR_RETURN( hr, L"FontBuffer" );
#pragma endregion

	return true;
}

void FontShader::TerminateShader()
{
	SAFE_RELEASE( m_pFontColorBuffer );
...
void FontShader::OutputShaderErrorMessage( ID3D10Blob * pBlob, HWND hWnd, WCHAR * fileName )
{
...
bool FontShader::SetShaderParam( ID3D11DeviceContext * pDeviceContext, const MatrixBuffer & matrixBuffer, ID3D11ShaderResourceView * pTexture, XMFLOAT4 fontColor )
{
	HRESULT hr;
	...
	// FONT_COLOR_BUFFER
	FontColorBuffer* pFontPtr = nullptr;
	
	hr = pDeviceContext->Map( m_pFontColorBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource );
	if( FAILED( hr ) )
		return false;

	pFontPtr = (FontColorBuffer*)mappedResource.pData;
	pFontPtr->_fontColor = fontColor;

	pDeviceContext->Unmap( m_pFontColorBuffer, 0 );

	bufferSlot = 0;
	pDeviceContext->PSSetConstantBuffers( bufferSlot, 1, &m_pFontColorBuffer );
